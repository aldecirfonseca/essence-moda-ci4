...
class Produto extends BaseController
{
    ...
    public function form($action = null, $id = null)
    {
        ...
        $this->dados['aCategoria']   = $this->CategoriaModel->where("statusRegistro", 1)->orderBy('descricao')->findAll();
				...
				if (($arq->isValid()) && !($arq->hasMoved())) {
					...
					if (array_search($extArquivo, array('bmp', 'png', 'jpg', 'jpeg', 'gif', 'webp')) === false) {
                        ...
                        return view("admin/formProduto" , [
                            'action'        => $post['action'],
                            'data'          => $post,
                            'aCategoria'    => $this->CategoriaModel->where("statusRegistro", 1)->orderBy('descricao')->findAll(),
                            'aAnexo'        => $aAnexo,
                            'errors'        => []
                        ]);
					}
